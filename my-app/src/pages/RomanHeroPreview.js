// Small always-on 3D preview shown in the Roman page hero — same pattern as
// JwstHeroPreview.js / HubbleHeroPreview.js (auto-orbiting camera, no
// OrbitControls). Roman's model is a single merged mesh with no per-part
// node hierarchy, so RomanFullscreen.js is a free-orbit explorer with a
// static facts panel rather than click-to-select hotspots.
...
function Scene({ onLoaded }) {
  ...
  // can only have one parent (see IssStationFullscreen.js for the full story).
...
export default function RomanHeroPreview({ onOpenFullscreen }) {
  ...
  return (
    <div className="tele3d-hero-wrap">
      ...
      <button type="button" className="tele3d-hero-cta" onClick={onOpenFullscreen} aria-label={t("roman.hero.open3dLabel")}>
        <span className="tele3d-hero-cta-ico">⛶</span>
        <span>{t("roman.hero.open3d")}</span>
      </button>
    </div>
  );
}
